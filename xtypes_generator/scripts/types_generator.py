...
def flatten_properties(properties: dict) -> dict:
    """
    Flattens the properties to JSON pointer key and value
    :param properties: the unflattened properties
    ...
    for top_key, top_value in properties.items():
        ...
        while len(unresolved) > 0:
            ...
            if ("type" not in current_value) or (isinstance(current_value["type"], dict)):
                ...
                continue
            # Found leaf property
            resolved[current_key] = current_value
    return resolved
...
def parse_yaml(yaml_data, project_name, lang: Language):
    ...
    if "properties" in yaml_data:
        resolved = flatten_properties(yaml_data["properties"])
